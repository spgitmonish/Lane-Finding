...
def process_image(image, showImages=0, frameCount=0, saveImages=0):
    ...
    if saveImages == 0:
        ...
        if showImages == 1:
            ...
            hsv1 = cv2.cvtColor(imageCopy, cv2.COLOR_BGR2HSV)
            hsv2 = cv2.cvtColor(imageCopy, cv2.COLOR_BGR2HSV)
            hsv = cv2.cvtColor(imageCopy, cv2.COLOR_BGR2HSV)
            plt.title("HSV")
            plt.imshow(hsv, cmap="hsv")
            plt.show()

            # Hue for yellow starts at 60(going conservative)
            # NOTE: H: 0 - 180, S: 0 - 255, V: 0 - 255 in OpenCV
            lower_yellow = np.array([60, 50, 50])
            upper_yellow = np.array([120, 255, 255])

            # Hue for white starts at 0, saturation starts at 0, value
            # is 100%(intensity)
            lower_white = np.array([0, 0, 0])
            upper_white = np.array([20, 255, 255])

            # Filter only by yellow
            maskY = cv2.inRange(hsv2, lower_yellow, upper_yellow)

            # HSV filtered
            hsv1 = cv2.bitwise_and(hsv1, hsv1, mask=maskY)
            imageCopy = cv2.cvtColor(hsv1, cv2.COLOR_HSV2BGR)
            plt.title("Yellow")
            plt.imshow(imageCopy)
            plt.show()

            # Filter only by white
            maskW = cv2.inRange(hsv2, lower_white, upper_white)
            # HSV filtered
            hsv2 = cv2.bitwise_and(hsv2, hsv2, mask=maskW)
            imageCopy = cv2.cvtColor(hsv2, cv2.COLOR_HSV2BGR)
            plt.title("White")
            plt.imshow(imageCopy)
            plt.show()
            ...
            imageCopy = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
            plt.title("BGR")
            plt.imshow(imageCopy)
            plt.show()
...
for frame in clip.iter_frames():
    print "\n\nFC: " + str(frameCount)
    if frameCount == 120:
        process_image(frame, 1, frameCount, 0)
